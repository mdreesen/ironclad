  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      ssrRenderList(__props.data, (item, i) => {
        _push(`<div class="col-span-1 md:col-span-4 border-t md:border-t-0 border-l border-r border-black md:last:border-r-0 p-6 md:p-8 first:border-t-0 hover:bg-black hover:text-white transition-colors duration-500 cursor-crosshair"><span class="text-[9px] font-bold tracking-[0.2em] uppercase block mb-8 md:mb-12">${ssrInterpolate(item.label)}</span><div class="flex items-baseline gap-2"><span class="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter tabular-nums">${ssrInterpolate(item.value)}</span><span class="text-[10px] font-bold uppercase">${ssrInterpolate(item.unit)}</span></div></div>`);
      });
      _push(`<!--]--></div>`);
    };
  }
...
//# sourceMappingURL=index-YjPPVwHN.mjs.map
